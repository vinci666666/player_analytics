-- Rebuild both Agent retention snapshots from the raw slot_parent_bet facts.
-- The transaction keeps readers on the previous snapshot until the refresh commits.
BEGIN;
...
GROUP BY bet_at::date, parent_agent_id, agent_id, slot_id, player_id;

CREATE INDEX ON _agent_player_game_daily
    (parent_agent_id, agent_id, slot_id, player_id, date);
CREATE INDEX ON _agent_player_game_daily
    (date, parent_agent_id, agent_id, slot_id);
ANALYZE _agent_player_game_daily;
...
SELECT
    ...
    SUM(odd_ratio_sum) AS odd_ratio_sum,
    SUM(odd_ratio_count)::INT8 AS odd_ratio_count,
    SUM(b1_spin)::INT8 AS b1_spin,
    SUM(b1_bet) AS b1_bet,
    SUM(b1_win) AS b1_win,
    SUM(b1_odd_sum) AS b1_odd_sum,
    SUM(b1_odd_count)::INT8 AS b1_odd_count,
    SUM(b2_spin)::INT8 AS b2_spin,
    SUM(b2_bet) AS b2_bet,
    SUM(b2_win) AS b2_win,
    SUM(b2_odd_sum) AS b2_odd_sum,
    SUM(b2_odd_count)::INT8 AS b2_odd_count,
    SUM(b3_spin)::INT8 AS b3_spin,
    SUM(b3_bet) AS b3_bet,
    SUM(b3_win) AS b3_win,
    SUM(b3_odd_sum) AS b3_odd_sum,
    SUM(b3_odd_count)::INT8 AS b3_odd_count
FROM _agent_player_game_daily
GROUP BY date, parent_agent_id, agent_id, player_id;
...
retention AS (
    ...
    LEFT JOIN _agent_player_daily r1
      ON r1.parent_agent_id = f.parent_agent_id AND r1.agent_id = f.agent_id
     AND r1.player_id = f.player_id AND r1.date = f.first_date + 1
    LEFT JOIN _agent_player_daily r3
      ON r3.parent_agent_id = f.parent_agent_id AND r3.agent_id = f.agent_id
     AND r3.player_id = f.player_id AND r3.date = f.first_date + 3
    LEFT JOIN _agent_player_daily r7
      ON r7.parent_agent_id = f.parent_agent_id AND r7.agent_id = f.agent_id
     AND r7.player_id = f.player_id AND r7.date = f.first_date + 7
...
financial AS (
    SELECT
        ...
        SUM(b1_spin)::INT8 AS b1_spin, SUM(b1_bet) AS b1_bet, SUM(b1_win) AS b1_win,
        SUM(b1_odd_sum) / NULLIF(SUM(b1_odd_count), 0) AS b1_odd_rtp,
        COUNT(*) FILTER (WHERE b2_spin > 0)::INT8 AS b2_players,
        SUM(b2_spin)::INT8 AS b2_spin, SUM(b2_bet) AS b2_bet, SUM(b2_win) AS b2_win,
        SUM(b2_odd_sum) / NULLIF(SUM(b2_odd_count), 0) AS b2_odd_rtp,
        COUNT(*) FILTER (WHERE b3_spin > 0)::INT8 AS b3_players,
        SUM(b3_spin)::INT8 AS b3_spin, SUM(b3_bet) AS b3_bet, SUM(b3_win) AS b3_win,
        SUM(b3_odd_sum) / NULLIF(SUM(b3_odd_count), 0) AS b3_odd_rtp
    FROM _agent_player_daily
    GROUP BY date, parent_agent_id, agent_id
)
SELECT
    f.date, f.parent_agent_id, f.agent_id,
    f.player_count, COALESCE(r.dnu, 0),
    COALESCE(r.retention_1, 0), COALESCE(r.retention_3, 0), COALESCE(r.retention_7, 0),
    f.total_spin_count, f.total_bet_amount, f.total_win_amount,
    COALESCE(f.total_win_amount / NULLIF(f.total_bet_amount, 0), 0), COALESCE(f.odd_rtp, 0),
    f.b1_players, COALESCE(f.b1_spin::NUMERIC / NULLIF(f.b1_players, 0), 0),
    f.b1_spin, f.b1_bet, f.b1_win,
    COALESCE(f.b1_win / NULLIF(f.b1_bet, 0), 0), COALESCE(f.b1_odd_rtp, 0),
    f.b2_players, COALESCE(f.b2_spin::NUMERIC / NULLIF(f.b2_players, 0), 0),
    f.b2_spin, f.b2_bet, f.b2_win,
    COALESCE(f.b2_win / NULLIF(f.b2_bet, 0), 0), COALESCE(f.b2_odd_rtp, 0),
    f.b3_players, COALESCE(f.b3_spin::NUMERIC / NULLIF(f.b3_players, 0), 0),
    f.b3_spin, f.b3_bet, f.b3_win,
    COALESCE(f.b3_win / NULLIF(f.b3_bet, 0), 0), COALESCE(f.b3_odd_rtp, 0)
FROM financial f
LEFT JOIN retention r USING (date, parent_agent_id, agent_id);

TRUNCATE TABLE public.agent_daily_game_retention;
...
WITH first_dates AS (
    SELECT parent_agent_id, agent_id, slot_id, player_id, MIN(date) AS first_date
    FROM _agent_player_game_daily
...
retention AS (
    SELECT
        f.first_date AS date,
        f.parent_agent_id, f.agent_id, f.slot_id,
        COUNT(*)::INT8 AS dnu,
        COUNT(r1.player_id)::NUMERIC / NULLIF(COUNT(*), 0) AS retention_1,
        COUNT(r3.player_id)::NUMERIC / NULLIF(COUNT(*), 0) AS retention_3,
        COUNT(r7.player_id)::NUMERIC / NULLIF(COUNT(*), 0) AS retention_7
    FROM first_dates f
    LEFT JOIN _agent_player_game_daily r1
      ON r1.parent_agent_id = f.parent_agent_id AND r1.agent_id = f.agent_id
     AND r1.slot_id = f.slot_id AND r1.player_id = f.player_id AND r1.date = f.first_date + 1
    LEFT JOIN _agent_player_game_daily r3
      ON r3.parent_agent_id = f.parent_agent_id AND r3.agent_id = f.agent_id
     AND r3.slot_id = f.slot_id AND r3.player_id = f.player_id AND r3.date = f.first_date + 3
    LEFT JOIN _agent_player_game_daily r7
      ON r7.parent_agent_id = f.parent_agent_id AND r7.agent_id = f.agent_id
     AND r7.slot_id = f.slot_id AND r7.player_id = f.player_id AND r7.date = f.first_date + 7
    GROUP BY f.first_date, f.parent_agent_id, f.agent_id, f.slot_id
),
financial AS (
    SELECT
        date, parent_agent_id, agent_id, slot_id,
        ...
        COUNT(*) FILTER (WHERE b3_spin > 0)::INT8 AS b3_players,
        SUM(b3_spin)::INT8 AS b3_spin, SUM(b3_bet) AS b3_bet, SUM(b3_win) AS b3_win,
        COUNT(*) FILTER (WHERE b1_spin > 0 AND b1_spin < 10)::NUMERIC AS d1,
        COUNT(*) FILTER (WHERE b1_spin >= 10 AND b1_spin < 20)::NUMERIC AS d2,
        COUNT(*) FILTER (WHERE b1_spin >= 20 AND b1_spin < 50)::NUMERIC AS d3,
        COUNT(*) FILTER (WHERE b1_spin >= 50 AND b1_spin < 100)::NUMERIC AS d4,
        COUNT(*) FILTER (WHERE b1_spin >= 100 AND b1_spin < 300)::NUMERIC AS d5,
        COUNT(*) FILTER (WHERE b1_spin >= 300 AND b1_spin < 500)::NUMERIC AS d6,
        COUNT(*) FILTER (WHERE b1_spin >= 500 AND b1_spin < 1000)::NUMERIC AS d7,
        COUNT(*) FILTER (WHERE b1_spin >= 1000)::NUMERIC AS d8
    FROM _agent_player_game_daily
    GROUP BY date, parent_agent_id, agent_id, slot_id
)
SELECT
    f.date, f.parent_agent_id, f.agent_id, f.slot_id,
    f.player_count, COALESCE(r.dnu, 0),
    COALESCE(r.retention_1, 0), COALESCE(r.retention_3, 0), COALESCE(r.retention_7, 0),
    f.total_spin_count, f.total_bet_amount, f.total_win_amount,
    f.b1_players, COALESCE(f.b1_spin::NUMERIC / NULLIF(f.b1_players, 0), 0),
    f.b1_spin, f.b1_bet, f.b1_win, COALESCE(f.b1_bet / NULLIF(f.b1_spin, 0), 0),
    f.b2_players, COALESCE(f.b2_spin::NUMERIC / NULLIF(f.b2_players, 0), 0),
    f.b2_spin, f.b2_bet, f.b2_win, COALESCE(f.b2_bet / NULLIF(f.b2_spin, 0), 0),
    f.b3_players, COALESCE(f.b3_spin::NUMERIC / NULLIF(f.b3_players, 0), 0),
    f.b3_spin, f.b3_bet, f.b3_win, COALESCE(f.b3_bet / NULLIF(f.b3_spin, 0), 0),
    COALESCE(f.d1 / NULLIF(f.b1_players, 0), 0),
    COALESCE(f.d2 / NULLIF(f.b1_players, 0), 0),
    COALESCE(f.d3 / NULLIF(f.b1_players, 0), 0),
    COALESCE(f.d4 / NULLIF(f.b1_players, 0), 0),
    COALESCE(f.d5 / NULLIF(f.b1_players, 0), 0),
    COALESCE(f.d6 / NULLIF(f.b1_players, 0), 0),
    COALESCE(f.d7 / NULLIF(f.b1_players, 0), 0),
    COALESCE(f.d8 / NULLIF(f.b1_players, 0), 0)
FROM financial f
LEFT JOIN retention r USING (date, parent_agent_id, agent_id, slot_id);

ANALYZE public.agent_daily_retention;
ANALYZE public.agent_daily_game_retention;
